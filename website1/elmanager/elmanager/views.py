...
def account_lilun_distinct_st(request):
	...
	sql="select distinct ac from P_BASIC_cplus where ac is not null"
	cplus_list=ms.dict_sql(sql)
	cplus_list=[item['ac'] for item in cplus_list]
	if acname in cplus_list:
		sql="select kka.AC,kka.P,kka.total_P,kka.ST,kka.stockdate,kka.ST as TradName from (select s.P,round((s.P*p.P_size*aa.ratio/100.0),1) as total_P,s.st,s.stockdate,p.AC,p.P_size,aa.ratio from real_st_report s inner join P_BASIC_cplus p on s.ST=p.ST inner join AC_RATIO aa  on p.AC=aa.AC and p.AC='%s' and s.stockdate<='%s'  ) kka  inner join ( select MAX(stockdate) as stockdate,s.st from real_st_report s inner join P_BASIC_cplus p on s.ST=p.ST inner join AC_RATIO aa  on p.AC=aa.AC and p.AC='%s' and s.stockdate<='%s'  group by s.ST  ) kkb on kka.stockdate=kkb.stockdate and kka.ST=kkb.ST   order by kka.P" % (acname,day,acname,day)
	else:
