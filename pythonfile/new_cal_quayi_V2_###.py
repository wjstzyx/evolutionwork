...
def main_fun():
	sql="SELECT id, [acname] ,[positionsymbol] ,[quanyisymbol] ,[iscaculate]  ,[isstatistic] FROM [LogRecord].[dbo].[quanyicaculatelist] where iscaculate=1 and issumps=0 and isyepan in (0,1,12) and acname like '%stepmultigaosheng1'  AND quanyisymbol not in ('IF','IC','IH') order by id desc "
	res=ms.dict_sql(sql)
	for item in res:
		print item['acname'],item['id']
		positionsymbol=item['positionsymbol']
