...
# sql="select  distinct top (30) convert(nvarchar(10),stockdate,120) as day from TSymbol where StockDate<='%s' order by day desc" % (equity_day)
...
def del_get_delta_info(symbol,symbolid,account):
	#获取成交回报
...
def main_get_account_position(account,step_acname):
	sql="select SUBSTRING(f_ac,0, CHARINDEX('StepMu',f_ac)) as symbol ,Stock from p_follow where ac='%s' order by f_ac" % (step_acname)
...
# cal_ac_day_equity('StepMultiI300w_up','yyStepMultiI_up',2.2)
# cal_ac_day_equity_real(account,52,'zc')
# cal_ac_day_equity_huibao(account,7,'yy')
# cal_ac_day_equity_huibao(account,52,'zc')
# cal_ac_day_equity_huibao(account,8,'zn')
# cal_ac_day_equity_huibao(account,34,'fg')
#
# # 计算理论当天权益
...
backup_account=['666061010','257188832']
if account in backup_account:
	pass
	sql="select F_ac,ratio/100.0 as ratio from p_follow where ac='%s'" % (account)
	res=ms.dict_sql(sql)
	if len(res)==1:

		step_acname=res[0]['F_ac']
		totalratio =res[0]['ratio']
		totalratio=2.2
		print step_acname

	else:
		print '#### Find more than one config,Please check '
		exit()

else:
	print '### This suit for account only config one StepMulti !!!'
	exit()
...
main_get_lilun(step_acname,  totalratio)
print 'step2 position equity'
main_get_account_position(account, step_acname)
print 'step3 Account huibao equity'
main_get_huibao_position(account, step_acname)
