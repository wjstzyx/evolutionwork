...
def daylycaculate(symbolfrom,symbolto,ac):
	...
	try:		
		...
		pass
	sql="select deltaposition,0 as position,C,0 as deltaquanyi,0 as comm, D,stockdate into %s from (select ISNULL(b.position,0) AS deltaposition ,a.StockDate,a.C,a.D from TSymbol a left outer join [quanyi_log_groupby_test] b on a.Symbol='%s'  and b.symbol='%s' and b.type=0  and b.ac='%s' and a.StockDate=b.stockdate where a.Symbol='%s' ) temp " % (tablename,symbolto,symbolfrom,ac,symbolto)
	print sql
	ms.insert_sql(sql)
...
def input_groupbyquanyi(ac,symbol,type,D):
	...
	sql="delete from quanyi_log_groupby_test where ac='%s' and symbol='%s' and type=%s and stockdate>'%s'" % (ac,symbol,type,deltime)
	ms.insert_sql(sql)
	#--end
	try:
	...
	if res>0:
		...
		sql="select top 1 totalposition,stockdate from [quanyi_log_groupby_test] where ac='%s' and symbol='%s' and TYPE=%s order by stockdate desc " % (ac,symbol,type)
		res=ms.dict_sql(sql)
		if res:
		...
		#计算昨天的总仓位
		sql="select top 1 totalposition,stockdate from [quanyi_log_groupby_test] where ac='%s' and symbol='%s' and TYPE=%s and stockdate<'%s' order by stockdate desc " % (ac,symbol,type,myDbegin)
		res=ms.dict_sql(sql)
		if res:
			lastdayP=res[0]['totalposition']
		else:
		...
		if firststockdate:
			...
			if firststockdate>laststockdate:
				sql="insert into [Future].[dbo].[quanyi_log_groupby_test](ac,symbol,[type],[position],[stockdate],[totalposition]) values "+valueslist
				ms.insert_sql(sql)
...
def pre_quanyi_data(ac,symbol,type):
	sql="select max(stockdate) as stockdate FROM [Future].[dbo].[quanyi_log_groupby_test] where ac='%s' and symbol='%s'" % (ac,symbol)
...
daylycaculate('RBnight','RBnight','RBQGSTREVYP_TG')