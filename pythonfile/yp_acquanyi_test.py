...
def input_groupbyquanyi(ac,symbol,type,D):
	...
	sql="delete from quanyi_log_groupby where ac='%s' and symbol='%s' and type=%s and stockdate>'%s'" % (ac,symbol,type,deltime)
	ms.insert_sql(sql)
	#--end
	try:
	...
	if res>0:
		...
		#计算昨天的总仓位
		sql="select top 1 totalposition,stockdate from [quanyi_log_groupby] where ac='%s' and symbol='%s' and TYPE=%s and stockdate<'%s' order by stockdate desc " % (ac,symbol,type,myDbegin)
		res=ms.dict_sql(sql)
		if res:
			lastdayP=res[0]['totalposition']
		else:
