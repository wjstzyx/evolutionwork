...
def test_record_num(ac,symbol):
	sql="select st_report.id,st_report.p,st_report.pp,p.symbol,st_report.stockdate,st_report.st,p.p_size,p.ac,p.ratio,p.d from st_report  inner join p_log p on p.st=st_report.st and p.ac='%s' and p.symbol='%s' and p.d=st_report.d and st_report.type=0 ORDER by stockdate,st,id" % (ac,symbol)
	# print sql
	st_log_record=ms.find_sql(sql)
	sql="select temp.* from Tsymbol o inner join  (select st_report.id,st_report.p,st_report.pp,p.symbol,st_report.stockdate,st_report.st,p.p_size,p.ac,p.ratio,p.d from st_report  inner join p_log p on p.st=st_report.st and p.ac='%s' and p.symbol='%s' and p.d=st_report.d and st_report.type=0) temp on temp.stockdate=o.stockdate and o.symbol=temp.symbol ORDER by stockdate,st,id" % (ac,symbol)
	# print sql
	st_log_Tsymbol_record=ms.find_sql(sql)
...
def main_test_IFlist():
	itemlist=('9CPPUD','9DayBrIF','9Distance','9DUD1','9DUDHL','9DUDRV','9EXR1410','9EXV1410','9FORCE','9FQS1','9HAL','9HAL2','9HAL3','9HALMA','9HUITIAO','9HUITIAO2','9HUITIAO3','9HUITIAO4','9KDHDAY','9KDHPM','9Linerate','9RATE','9MALONGK','9MiddayTrend','9MinVolPbuy','9MONDAY','9MORNINGOUT','9MT','9MVRATE','9NHL','9NOON','9OpenBet','9QMA','9QPMIF','9Reversal','9Reversal2','9Reversal3','9V4EIV','9VK1','9VK3','9VPINVOl_L','9VPINVOl_S','9VPINVOl_S2','9wb','9weipan','9WeipanREV','9WeipanStatics','9YAP01','9YY2','9YYMA','9LUD','9LUD2CH','9LUD3','9LUD4','9LUD5','9LUD6','9LUD7','9LUD8','9LUD10','9LUD11V2','9LUD13','9LUD14','9LUD16','9LUDCH1','9LUDCH4','9LUDCH5','9LUDCH6','9LUDCH8','LUD52015','LUD62015','LUD72015','LUD82015','TimeV2Pm','TimeV3DtaPm','TimeV3HLAm','TimeV3HLPm','V4EIVelements','V4EIVNEW','IFQG1310','IFQGEX','IFQGOT','9QGBombma2','IFQGTB','IFQGTR','IFQGWB','YEQGTR','YEQGEX','YEQGOT','YEQGZH211')
	symbol='IF'
	i=0
	itemlist=['IFQGWB']
...
def main_test_AGlist():
	itemlist=('9AGOLD','9AGVD05','9AGVD06','AGNEW4','AGNEW6','AGNEW8','AGNEW19','AGNEWLVO')	
	symbol='AG'
	i=0
	# itemlist=['IFQGWB']
	for item in itemlist:
...
def main_test_CUlist():
	itemlist=('CUDUDHL','ESPcu','CU-LKVCU1','CU-LKVCU2','CUPUDCU','QCU18MIN','QPMCU','CUVk2CU','CUVK3')	
	symbol='CU'
	i=0
	# itemlist=['IFQGWB']
	for item in itemlist:
...
def main_test_RBlist():
	itemlist=('Rb_QGpLud','RbCX_QGRev','RbCX_QGtr','RB_CXVolume','RB_Daybreaker','RB_LiangtuPipei','RB_LRC_Trend','RB_LUD','RB_MT','RB_RBreaker','RB_RSI','RB_ST_Reversal','RB_ST_Trend','RB_VPIN','RB_ZhixianPipei','CH4RBZS','DAYGAPRB','RBHAL','PUDRB','RBSV','UDKRB','V7RB')
	symbol='RB'
	i=0
	# itemlist=['IFQGWB']
	for item in itemlist:
