...
def main_fun():
	...
	sql="SELECT id, [acname] ,[positionsymbol] ,[quanyisymbol] ,[iscaculate]  ,[isstatistic] FROM [LogRecord].[dbo].[quanyicaculatelist] where iscaculate=1 and issumps=0 and isyepan in (0,1,12)  AND quanyisymbol not in ('IF','IC','IH')  and acname in (SELECT f_ac   FROM [Future].[dbo].[p_follow] where ac ='StepMultigaosheng2')order by id desc "
