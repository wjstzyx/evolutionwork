...
def input_groupbyquanyi(ac,symbol,type,nowD):
	#删除当日已经存在的记录(8点之后)
	...
	#print sql
	ms.insert_sql(sql)
	sql="select * into  #temp_quanyi_new from (select '%s' as ac,'%s' as symbol,'%s' as type,temp.id,p,PP,p_size,ratio,st,o.stockdate from tsymbol o inner join (select st_report.id,st_report.p,st_report.pp,p.symbol,st_report.stockdate,st_report.st,p.p_size,p.ac,p.ratio,st_report.type from st_report  inner join #temp_p_log p on p.st=st_report.st and p.ac='%s' and p.symbol='%s' and st_report.type=%s ) temp on temp.stockdate=o.stockdate and o.symbol=temp.symbol where o.symbol='%s' ) temp " % (ac,symbol,type,ac,symbol,type,symbol)
	#print sql
	ms.insert_sql(sql)
...
def pre_quanyi_data(ac,symbol,type):
	...
	if res['stockdate'] == None:
		...
		nowD=int(nowD)
	input_groupbyquanyi(ac,symbol,type,nowD)
