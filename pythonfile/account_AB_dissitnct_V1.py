...
#################general steps ########################
### setp 1 获取原始仓位序列 #####
def get_origin_position_list(tablename,p_followac,acname,ratio=1):
	sql="select q.ClosePrice,q.stockdate,round(q.totalposition*mr.ratio*%s,0) as totalposition ,q.symbol,sid.S_ID from p_follow p inner join %s q on p.F_ac=q.AC and p.AC='%s' inner join LogRecord.dbo.test_margin mr on q.symbol=mr.symbol inner join symbol_id sid on q.symbol=sid.Symbol where q.AC='%s' and stockdate>='%s' and stockdate<='%s' order by stockdate" % (ratio,tablename,p_followac,acname,begintime,endtime)
...
def cal_ac_day_equity(tablename,p_followac,acname,ratio=1):
	postionpd, symbol = get_origin_position_list(tablename,p_followac, acname, ratio)
	#获取 指数 价格 系列权益
	totalpo = get_Tsymbol_by_symbol(symbol, postionpd)
	newtotalpo = cal_equity(symbol, totalpo)
...
def main_get_ab_backtest(tablename,step_acname,totalratio):
	sql="select * from (select distinct f_ac from p_follow where ac='%s') a order by f_ac" % (step_acname)
	myres=ms.dict_sql(sql)
	for myitem in myres:
		#print myitem
		cal_ac_day_equity(tablename,step_acname,myitem['f_ac'],totalratio)
...
#mytype= 'lilun' 'position' 'huibao'
mytype="ab_backtest"
if mytype=='ab_backtest':
	# 计算理论当天权益
	tablename='quanyi_log_groupby_v3'
	main_get_ab_backtest(tablename,step_acname,totalratio)
if mytype=='ab_real_st':
	# 计算理论当天权益
	tablename = 'real_quanyi_log_groupby_v2'
	main_get_ab_backtest(tablename,step_acname,totalratio)
if mytype=='position':	# #
	# # 计算期货账户仓位 当天权益
