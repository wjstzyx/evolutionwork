...
def read_position_from_csv(symbol,ratio=1):
	dir=r'C:\Users\YuYang\Documents\Tencent Files\794513386\FileRecv'+'\\'+symbol+'.csv'
	df1=pd.read_csv(dir)
	df1=df1[['Date/Time','typC','Close','ps','Ticker']]
	symbol=df1.iloc[-1]['Ticker']
	df1['stockdate']=pd.to_datetime(df1['Date/Time'],format='%Y/%m/%d %H:%M:%S')
	df1['CZL']=df1['typC']
	df1['C'] = df1['Close']
	df1['totalposition'] = df1['ps']*ratio
	return df1[['stockdate','C','CZL','totalposition']],symbol



def cal_ac_day_equity(symbol,p_followac,acname,ratio=1):

	#postionpd, symbol = get_origin_position_list(p_followac, acname, ratio)
	#获取 指数 价格 系列权益
	#totalpo = get_Tsymbol_by_symbol(symbol, postionpd)
	#替换成 诚心仓位
	totalpo,symbol = read_position_from_csv(symbol,ratio=1)
...
def main_get_lilun(step_acname,totalratio):
	sql="select * from (select distinct f_ac from p_follow where ac='%s' and f_ac='csStepMultiI_up') a order by f_ac" % (step_acname)
	myres=ms.dict_sql(sql)
	for myitem in myres:
		#print myitem
		cal_ac_day_equity('CS',step_acname,myitem['f_ac'],totalratio)
		exit()
