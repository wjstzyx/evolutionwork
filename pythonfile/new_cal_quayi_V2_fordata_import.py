...
def main_fun_sumps():
	#获取需要处理的列表
	sql="SELECT id, [acname] ,[positionsymbol] ,[quanyisymbol] ,[iscaculate]  ,[isstatistic] FROM [LogRecord].[dbo].[quanyicaculatelist] where iscaculate=1 and acname in (select distinct ac from P_BASIC where st in (select distinct st from st_report_test)) and issumps=1 and isyepan in (0,1,12) order by id desc "
...
def main_fun():
	#获取需要处理的列表
	sql="SELECT id, [acname] ,[positionsymbol] ,[quanyisymbol] ,[iscaculate]  ,[isstatistic] FROM [LogRecord].[dbo].[quanyicaculatelist] where iscaculate=1 and acname in (select distinct ac from P_BASIC where st in (select distinct st from st_report_test)) and issumps=0 and isyepan in (0,1,12) order by id desc "
	#sql="SELECT top 17 id,[acname] ,[positionsymbol] ,[quanyisymbol] ,[iscaculate]  ,[isforbacktest]  ,[isstatistic] FROM [LogRecord].[dbo].[quanyicaculatelist] where quanyisymbol in ('RB') and iscaculate=1 order by sortnum"
	res=ms.dict_sql(sql)
	for item in res:
		print item['acname'],item['id']
