...
def input_groupbyquanyi(ac,symbol):
	...
	sql="select * into  #temp_quanyi_new from ( select p.ac,p.symbol,0 as type,[LogRecord].[dbo].[Trading_logTrade].id,[LogRecord].[dbo].[Trading_logTrade].p,[LogRecord].[dbo].[Trading_logTrade].pp,p.p_size,p.ratio ,[LogRecord].[dbo].[Trading_logTrade].st,[LogRecord].[dbo].[Trading_logTrade].systemtime  as stockdate from [LogRecord].[dbo].[Trading_logTrade]  inner join #temp_p_log p on p.st=[LogRecord].[dbo].[Trading_logTrade].st and p.ac='%s' and p.symbol='RBnight')temp " % (ac)
	print sql
	ms.insert_sql(sql)
