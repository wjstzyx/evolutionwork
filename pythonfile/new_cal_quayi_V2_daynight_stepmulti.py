...
def delete_info_from_V2(stepname):
	sql="delete from quanyi_log_groupby_v2 where ac in (select f_ac from p_follow where ac='%s' )" % (stepname)
	ms.insert_sql(sql)
	print 'delete quanyi_log_groupby_v2'
	sql="delete from dailyquanyi_V2 where ac in (select f_ac from p_follow where ac='%s' )" % (stepname)
	ms.insert_sql(sql)
	print 'delete dailyquanyi_V2'


def main_fun(stepname):
...
stepmultilist=['StepMultidnhiboth','StepMultidnhiprofit','StepMultidnhisharp','StepMultidnshort']
