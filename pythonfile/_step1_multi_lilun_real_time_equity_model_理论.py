...
#################general steps ########################
### setp 1 获取原始仓位序列 #####
def get_origin_position_list(stname,symbol,ratio=1):
	sql="SELECT 0 as ClosePrice,0 as S_ID,stockdate,'%s' as symbol,  [P]*%s as totalposition  FROM [Future].[dbo].[real_st_report] where st='%s' order by stockdate" % (symbol,ratio,stname)
	#print sql
...
def cal_ac_day_equity(stname,symbol,ratio=1):
	postionpd, symbol = get_origin_position_list(stname, symbol, ratio)
	#获取 指数 价格 系列权益
	totalpo = get_Tsymbol_by_symbol(symbol, postionpd)
	newtotalpo = cal_equity(symbol, totalpo)
...
if mytype=='lilun':
	# 计算理论当天权益
	cal_ac_day_equity('CS_CXV5_11_stair1up_', 'Cs', 1)
if mytype=='position':	# #
	# # 计算期货账户仓位 当天权益
